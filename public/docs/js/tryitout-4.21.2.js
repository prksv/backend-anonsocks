...
function tryItOut(endpointId) {
    ...
    document
        .querySelectorAll(`input[data-endpoint=${endpointId}],label[data-endpoint=${endpointId}]`)
    ...
    document.querySelectorAll(`#form-${endpointId} details`).forEach((el) => (el.open = true));
}
...
function cancelTryOut(endpointId) {
    ...
    document.querySelectorAll(`#form-${endpointId} details`).forEach((el) => (el.open = false));
    const authElement = document.querySelector(`#auth-${endpointId}`);
...
function makeAPICall(method, path, body = {}, query = {}, headers = {}, endpointId = null) {
    ...
    Object.keys(query).forEach((key) =>
        addItemToSearchParamsObject(key, query[key], url.searchParams)
    );
    ...
    return fetch(url, {
        ...
        Promise.all([response.status, response.statusText, response.text(), response.headers])
    );
}
...
function handleResponse(endpointId, response, status, headers) {
    ...
    responseContentEl.textContent =
        response === "" ? responseContentEl.dataset.emptyResponseText : response;
...
async function executeTryOut(endpointId, form) {
    ...
    urlParameters.forEach(
        (el) => (path = path.replace(new RegExp(`\\{${el.name}\\??}`), el.value))
    );

    const headers = Object.fromEntries(
        Array.from(form.querySelectorAll("input[data-component=header]")).map((el) => [
            el.name,
            el.value,
        ])
    );
